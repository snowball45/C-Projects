...
private:
    ...
    void upheap(size_type index) {
        // TODO
        if (index == 0){return;}
        T& parent1 = c[parent(index)];
        T& current = c[index];
        if (comp(parent1, current)){std::swap(c[index], c[parent(index)]); upheap(parent(index));}
        else {return;}
    }
    ...
    void downheap(size_type index) {
        ...
        if (right_child(index) >= c.size() && comp(c[index], c[left_child(index)])){std::swap(c[index], c[left_child(index)]); return;}
        else if (right_child(index) >= c.size() && comp(c[left_child(index)], c[index])){return;}

        if (comp(c[index], c[left_child(index)]) && comp(c[index], c[right_child(index)])){
            if (comp(c[left_child(index)], c[right_child(index)])){std::swap(c[right_child(index)], c[index]); downheap(right_child(index));}
            else {std::swap(c[left_child(index)], c[index]); downheap(left_child(index));}
        }
        else if (comp(c[index], c[left_child(index)])){std::swap(c[index], c[left_child(index)]); downheap(left_child(index));}
        else if (comp(c[index], c[right_child(index)])){std::swap(c[index], c[right_child(index)]); downheap(right_child(index));}
        else {return;}
    }
...
public:
     ...
     */
    const_reference top() const {const_reference ref = c[0]; return ref;}
